...
process getInputDirectories {
    ...
    if (params.comprehensive) {
      """
      mkdir flat_input
      cp `find -L $indir -name \"*CG.mk\"` flat_input/
      cp `find -L $indir -name \"*CHG.mk\"` flat_input/
      cp `find -L $indir -name \"*CHH.mk\"` flat_input/
      """
    } else {
      """
      mkdir flat_input
 ...
 */
  ...
  process convertToMethylExtract {
      publishDir "${params.outdir}/MethylExtract_profiles", mode: 'copy'

      input:
      file infile from methylation_profiles.flatten()

      output:
      file "*.output" into methylextract_profiles
      ...
      infile = \"${infile}\"
      outfile = infile.replace(\".mk\", \".output\")

      if infile.endswith(\"CG.mk\"):
        cmd = f\"methylKitToMe --infile {infile} --outfile {outfile} --context CG\"
      elif infile.endswith(\"CHG.mk\"):
        cmd = f\"methylKitToMe --infile {infile} --outfile {outfile} --context CHG\"
      elif infile.endswith(\"CHH.mk\"):
        cmd = f\"methylKitToMe --infile {infile} --outfile {outfile} --context CHH\"
      else:
